...
class Healthcare extends EventEmitter {
  ...
  init() {
    ...
    const setupDemocracy = (metadata) => {
      ...
      const handleRemoval = (data) => {
        ...
        this.groups
          .filter(g => g.tags.every(tag => server.tags.includes(tag)))
      ...
      this.democracy.on('elected', () => {
        ...
        // Get the data for this server.
        const server = this.getServer(metadata.data);
        ...
        // If a floating IP is specified, assing it to this server.
        this.groups
          .filter(g => g.tags.every(tag => server.tags.includes(tag)))
          .forEach((group) => {
            if (group.floatingIp && group.floatingIp !== '') {
              this.digitalocean.floatingIpsRequestAction(group.floatingIp, {
                type: 'assign',
                droplet_id: server.id,
              })
                .then(() => {
                  debug(`Assigned floating IP ${group.floatingIp} to ${server.privateIp}.`);
                })
                .catch(debug);
            }
          });
  ...
  balance(group) {
    ...
    const servers = this.servers.filter(s => s.tags.every(tag => group.tags.includes(tag)));
