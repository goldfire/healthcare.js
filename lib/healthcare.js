...
class Healthcare extends EventEmitter {
  ...
  init() {
    ...
    const getDroplets = () => {
      return this.digitalocean.dropletsGetAll({
        ...
        res.body.droplets.forEach((server) => {
    ...
    const getServerTags = (metadata) => {
      ...
      return this.digitalocean.dropletsGetById(this.id).then((data) => {
        this.tags = data.body.droplet.tags || [];
      });
    };
    ...
    const setupDemocracy = () => {
      return new Promise((resolve) => {
        ...
        this.democracy.on('added', (data) => {
          debug(`Server added to pool [${data.id}]:`, data);
        ...
        this.democracy.on('removed', (data) => {
          debug(`Server removed from pool [${data.id}]:`, data);
  ...
  group({tags = [], size = 1, floatingIp, config}) {
    ...
    if (floatingIp && floatingIp !== '' && thisServerInGroup) {
      ...
      group.democracy.on('elected', (data) => {
        debug(`New group leader [${this.id}]:`, data);
        ...
        this.digitalocean.floatingIpsRequestAction(group.floatingIp, {
          ...
          .catch((err) => {
            debug('Assign Floating IP Error:', err);
          });
      });
      group.democracy.on('removed', (data) => {
        debug(`Group member removed [${this.id}]:`, data);
      });
    }
  ...
  destroy(id) {
    ...
    return this.digitalocean.dropletsDelete(id).catch((err) => {
  ...
  create(config) {
    ...
    return this.digitalocean.dropletsCreate({
      ...
      debug('Server created:', data ? data.body : null);
    }).catch((err) => {
